...
// 初始化测试脚本内容
function initializeTestScript(url, browser = 'msedge') {
  const scriptLines = [
...
function generatePythonScript(url, actions, filename) {
  ...
  if (!filename || typeof filename !== 'string') {
    filename = 'test-' + Date.now() + '.js';  // 改为.js扩展名
  }
  ...
  let scriptLines = initializeTestScript(url, browser);
...
function generateNodeScript(url, actions, filename) {
  return generatePythonScript(url, actions, filename);  // 复用相同逻辑
}

module.exports = {
  log,
  generatePythonScript, // 保留原函数名，但生成Node.js脚本
  generateNodeScript
};